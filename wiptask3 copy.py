...
A menu-driven console program for a small shop that hires fishing and camping equipment.

Design notes:
- Monetary values are expressed in integer pence to avoid floating-point rounding issues.
- `CATALOG` encodes the immutable price list; related multipliers document the pricing rules.
- Pricing rules: first night at 100%; each additional night at 50%; late returns incur an
    extra 50% night.
- Input helpers normalise and validate user input with targeted error messages.
- Documentation adheres to PEP 257 docstrings to keep rationale close to the code.
- No imports are used, satisfying the assignment constraint.
"""
...
def read_yes_no(prompt: str = "(y/n): ") -> str:
    """Read and normalise a yes/no answer to canonical ``'y'`` or ``'n'``."""
    while True:
...
def calc_line_costs(daily_p: int, qty: int, nights: int, returned_on_time: bool) -> tuple[int, int, int]:
    """Return the per-line cost tuple ``(first_night, additional, delay)`` in pence.

    Args:
        daily_p: Daily price per unit in pence.
        qty: Quantity hired.
        nights: Number of nights hired.
        returned_on_time: Whether the items were returned by 14:00 the next day.
    """
...
def _parse_item_line(raw: str) -> tuple[str, int]:
    ...
    Args:
        raw: The raw entry typed by the user.
...
def read_item_lines(nights: int, returned_on_time: bool) -> list[dict]:
    ...
    Args:
        nights: The number of nights for the hire.
        returned_on_time: Whether the rental was returned by the 14:00 deadline.

    Returns:
        A non-empty list of dictionaries describing each hire line.
    """
...
def run_hire_flow(state: AppState) -> None:
    ...
    Args:
        state: The shared :class:`AppState` instance.
    """
    while True:
...
def run_earnings_report(state: AppState) -> None:
    ...
    Args:
        state: The shared :class:`AppState` instance containing hire records.
    """
